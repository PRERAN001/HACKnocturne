...
async function seed() {
  try {
    // ── Safety: block seed on production databases ──────────────
    if (process.env.NODE_ENV === 'production') {
      console.error('❌ Seed script blocked: NODE_ENV is "production".');
    ...
    if (uri.includes('production') || uri.includes('prod.')) {
      console.error('❌ Seed script blocked: MONGODB_URI appears to point at a production database.');
      process.exit(1);
    }

    if (!process.argv.includes('--confirm')) {
      console.error('⚠️  This will DELETE all sessions and businesses from the database.');
      console.error('   Run with --confirm to proceed:  node seed.js --confirm');
      process.exit(1);
    }
    // ────────────────────────────────────────────────────────────
